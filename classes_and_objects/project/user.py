class User:
    ...
    def get_book(self, author, book_name, days_to_return, library):
        if book_name not in library.books_available[author]:
            current_book_days_to_return = [k[book_name] for k in library.rented_books.values() if book_name in k]
            current_book_days_to_return = current_book_days_to_return[0]
            return f"The book \"{book_name}\" is already rented and will be available in {current_book_days_to_return} days!"
