class PizzaDelivery:
    ...
    def make_order(self):
        self.ordered = True
        pizza_dict = ', '.join([f"{ingredient}: {self.ingredients[ingredient]}" for ingredient in self.ingredients])

        return f"You've ordered pizza {self.name} prepared with {pizza_dict} and the price will be {self.price}lv."
